...
rinitpdyn <- function(dms,srec,psi,M,mata,wta,sela,hstart) {
  ...
  for(s in 1:ns) {
    ...
    if(s < srec) neqm[0,s,] <- 0
    if(s == srec) neqm[0,s,] <- c(psi,1-psi)
    if(s > srec) neqm[0,s,] <- n[0,s-1,]*exp(-M)
  ...
  for(s in 1:ns) {
    ...
    if(s > srec) {
      ...
      for(g in 1:2) neqm[0,s,g] <- n[0,s-1,g]*exp(-M)*(1-hsum[0,s-1,g])
